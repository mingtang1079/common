...
public class AddressSelector extends LinearLayout implements View.OnClickListener {
    ...
    private ArrayList<City> cities;//adapter 绑定的数据
    private ArrayList<City> Allcities;//所有的数据
    ...
    private int positon_one;//第一列
    private int positon_two;//第二列位置
     ...
     * 设置列表的数据源，设置后立即生效
     */
    public void setAllCitiys(ArrayList<City> mAllCitiys) {
        this.Allcities = mAllCitiys;
        setCities(mAllCitiys);
    }
    ...
    public void onClick(View v) {
        ...
        if (tabIndex == 0) {
            ...
            setCities(Allcities.get(positon_one).childs);
        ...
        public void onBindViewHolder(MyViewHolder holder, final int position) {
            ...
            holder.itemView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (tabIndex == 0) {
                        positon_one = position;//记录第一列城市的位置
                        setCities(Allcities.get(position).childs);

                    } else if (tabIndex == 1) {
                        positon_two = position;
                        setCities(Allcities.get(positon_one).childs.get(positon_two).childs);
                    } else if (onItemClickListener != null) {
                        //说明是最后一个了
                        if ((cities.get(position).childs == null || cities.get(position).childs.size() == 0))
                            onItemClickListener.itemClick(Allcities.get(positon_one), Allcities.get(positon_one).childs.get(positon_two), cities.get(position));
